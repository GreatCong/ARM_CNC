...
//		
		HAL_GPIO_TogglePin(LED_STATE_GPIO_Port,LED_STATE_Pin);//翻转LED
		HAL_Delay(30);
	}
...
{
	Protocol_ExecRtSystem();//检查各种状态 (包含了prepare motor)

	if(sys.suspend) {
		Protocol_ExecRtSuspend();//处理状态
	}
}
...
void Protocol_ExecRtSystem(void)
{
				...
				if((sys.state == STATE_IDLE) || ((sys.state & STATE_HOLD) && (sys.suspend & SUSPEND_HOLD_COMPLETE))) {
					if (sys.state == STATE_HOLD && sys.spindle_stop_ovr) {
						...
						if(Planner_GetCurrentBlock() && BIT_IS_FALSE(sys.suspend, SUSPEND_MOTION_CANCEL)) {
							...
							Stepper_PrepareBuffer(); // Initialize step segment buffer before beginning cycle. //如果执行块不为空，就开启计算步进电机的参数
							Stepper_WakeUp();//开启步进电机的中断
						}
	...
	if(sys.state & (STATE_CYCLE | STATE_HOLD | STATE_SAFETY_DOOR | STATE_HOMING | STATE_SLEEP| STATE_JOG)) {
		Stepper_PrepareBuffer();//准备计算步进电机参数
	}
}
