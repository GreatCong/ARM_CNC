...
void Report_FeedbackMessage(uint8_t message_code)
{
	...
	case MESSAGE_CRITICAL_EVENT:
		Printf_MSG("Reset to continue");
		break;

	case MESSAGE_ALARM_LOCK:
		Printf_MSG("'$H'|'$X' to unlock");
		break;

	case MESSAGE_ALARM_UNLOCK:
		Printf_MSG("Caution: Unlocked");
		break;
	...
	case MESSAGE_SAFETY_DOOR_AJAR:
		Printf_MSG("Check Door");
		break;

	case MESSAGE_CHECK_LIMITS:
		Printf_MSG("Check Limits");
		break;
	...
	case MESSAGE_RESTORE_DEFAULTS:
		Printf_MSG("Restoring defaults");
		break;

	case MESSAGE_SPINDLE_RESTORE:
		Printf_MSG("Restoring spindle");
		break;
...
void Report_GrblHelp(void) {
	Printf_MSG("[HLP:$$ $# $G $I $N $x=val $Nx=line $J=line $SLP $C $X $H ~ ! ? ctrl-x]\r\n");
}
...
void Report_ProbeParams(void)
{
	...
	Putc(':');
	Printf_MSG("%d", sys.probe_succeeded);
	report_util_feedback_line_feed();
}
...
void Report_NgcParams(void)
{
	...
	Printf_MSG("[G92:"); // Print G92,G92.1 which are not persistent in memory
	Report_AxisValue(gc_state.coord_offset);
	report_util_feedback_line_feed();
	Printf_MSG("[TLO:"); // Print tool length offset value
...
void Report_GCodeModes(void)
{
	...
	if(gc_state.modal.motion >= MOTION_MODE_PROBE_TOWARD) {
		Printf_MSG("38.");
		Printf_MSG("%d", gc_state.modal.motion - (MOTION_MODE_PROBE_TOWARD-2));
	}
	else {
		Printf_MSG("%d", gc_state.modal.motion);
	}

	report_util_gcode_modes_G();
	Printf_MSG("%d", gc_state.modal.coord_select+54);

	report_util_gcode_modes_G();
	Printf_MSG("%d", gc_state.modal.plane_select+17);

	report_util_gcode_modes_G();
	Printf_MSG("%d", 21-gc_state.modal.units);

	report_util_gcode_modes_G();
	Printf_MSG("%d", gc_state.modal.distance+90);

	report_util_gcode_modes_G();
	Printf_MSG("%d", 94-gc_state.modal.feed_rate);
	...
	if(gc_state.modal.program_flow) {
		...
		case PROGRAM_FLOW_COMPLETED_M30:
			Printf_MSG("%d", gc_state.modal.program_flow);
			break;
...
{
	Printf_MSG("[VER: %s %s:", GRBL_VERSION, GRBL_VERSION_BUILD);
	Printf_MSG("%s", line);
	report_util_feedback_line_feed();
	Printf_MSG("[OPT:"); // Generate compile-time build option list
    Putc('V');
	...
	Putc(',');
	Printf_MSG("%d", BLOCK_BUFFER_SIZE-1);
	Putc(',');
	Printf_MSG("%d", LINE_BUFFER_SIZE);
...
void Report_RealtimeStatus(void)
{
	...
	case STATE_JOG: Printf_MSG("Jog"); break;
	case STATE_HOMING: Printf_MSG("Home"); break;
	case STATE_ALARM: Printf_MSG("Alarm"); break;
	case STATE_CHECK_MODE: Printf_MSG("Check"); break;
	...
	if(BIT_IS_TRUE(settings.status_report_mask, BITFLAG_RT_STATUS_BUFFER_STATE)) {
		Printf_MSG("|Bf:");
		Printf_MSG("%d", Planner_GetBlockBufferAvailable());
		Putc(',');
		Printf_MSG("%d", FifoUsart_Available(STDOUT_NUM));
	}
#endif
	...
	if(sys.report_ovr_counter > 0) {
		...
		Printf_MSG("|Ov:");
		Printf_MSG("%d", sys.f_override);
		Putc(',');
		Printf_MSG("%d", sys.r_override);
		Putc(',');
		Printf_MSG("%d", sys.spindle_speed_ovr);
